...
async function getYouTubeData(url, onProgressUpdate, debugLogs = []) {
  ...
  // ===== LAYER 2: youtubei.js (most reliable for cloud environments) =====
  if (!rawItems || rawItems.length === 0) {
    debugLogs.push(`[Layer 2] Trying youtubei.js...`);
  ...
  // ===== LAYER 3: YoutubeTranscript library with NATIVE fetch (no custom wrapper) =====
  ...
  if (!rawItems || rawItems.length === 0) {
    ...
    for (const lang of langAttempts) {
      ...
      try {
        ...
        debugLogs.push(`[Layer 3] YoutubeTranscript.fetchTranscript (native fetch, lang=${lang || 'auto'})...`);
        ...
        debugLogs.push(`[Layer 3] lang=${lang || 'auto'} failed: ${e1.message}`);
      }
    }
  }

  // ===== LAYER 4: YoutubeTranscript with custom fetch wrapper (SOCS cookies) =====
  if (!rawItems || rawItems.length === 0) {
    for (const lang of langAttempts) {
      if (rawItems && rawItems.length > 0) break;
      try {
        const opts = lang ? { lang, fetch: vercelCustomFetch } : { fetch: vercelCustomFetch };
        debugLogs.push(`[Layer 4] YoutubeTranscript.fetchTranscript (custom fetch, lang=${lang || 'auto'})...`);
        rawItems = await YoutubeTranscript.fetchTranscript(videoId, opts);
        if (rawItems && rawItems.length > 0) {
          debugLogs.push(`[Layer 4 SUCCESS] Extracted ${rawItems.length} items with lang=${lang || 'auto'}`);
        }
      } catch (e2) {
        debugLogs.push(`[Layer 4] lang=${lang || 'auto'} failed: ${e2.message}`);
      }
    }
  }

  // ===== LAYER 5: InnerTube ANDROID Multi-Client Extractor =====
  if (!rawItems || rawItems.length === 0) {
    debugLogs.push(`[Layer 5] Trying InnerTube ANDROID Multi-Client...`);
  ...
  if (!rawItems || rawItems.length === 0) {
    ...
    try {
      ...
      debugLogs.push(`[Layer 6 Error] ${e4.message}`);
    }
  }
  ...
  if (!rawItems || rawItems.length === 0) {
    debugLogs.push(`[getYouTubeData Error] ALL 7 LAYERS FAILED for videoId: ${videoId}`);
