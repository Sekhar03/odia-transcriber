...
async function processMicSpeech(rawText, sourceLang = 'en-US', targetLang = 'or', audioData = null, mimeType = 'audio/webm') {

  if (!rawText && !audioData) {
  ...
  if (!GEMINI_API_KEY) {
    ...
    const translatedText = await translateSingleText(textToTranslate, targetLang);
    return {
      cleanedText: textToTranslate,
      translatedText
    };
  }
  ...
  try {
    ...
    const prompt = `You are an expert audio transcription analyzer, editor, and translator.
Task:
1. Listen to the provided audio file. Correct any spelling errors, grammatical slips, or muffled words due to background noise or accent. Reconstruct the speaker's true words into clean, coherent text in its original spoken language: ${sourceLang}.
2. Translate the corrected transcription into the target language: ${langLabel}.

Respond strictly in JSON format matching this schema:
{
  "cleanedText": "The corrected original speech text in its spoken language",
  "translatedText": "The translation of the cleanedText into the target language"
}`;

    const textPrompt = `You are an expert audio transcription analyzer and editor.
...
Respond strictly in JSON format matching this schema:
{
  ...
  "translatedText": "The translation of the cleanedText into the target language"
    ...
    return {
      cleanedText: aiData.cleanedText || rawText || 'Unrecognized speech',
