...
/**
 * Bulletproof Sanitizer for PDF rendering:
 * Guarantees mathematically that ZERO blank boxes (□) can appear in the PDF!
 */
function sanitizeTextForOdiaPdf(text) {
  if (!text) return '';
  let str = String(text);

  // 1. Convert smart quotes, dashes, currency & music symbols to clean ASCII
  str = str.replace(/[\u201C\u201D\u201E\u201F]/g, '"')
           .replace(/[\u2018\u2019\u201A\u201B]/g, "'")
           .replace(/\u2026/g, '...')
           .replace(/[\u2013\u2014]/g, '-')
  ...
  // 2. Untranslated Hindi state/place names to Odia transliteration
  const devanagariMap = {
  ...
  // 3. Strip remaining Devanagari & unmapped Unicode control chars
  str = str.replace(/[\u0900-\u097F]/g, '');
  str = str.replace(/[\u2000-\u206F\u20A0-\u20CF\uFEFF]/g, '');

  // 4. STRICT FILTER: Keep ONLY valid Odia Unicode (\u0B00-\u0B7F) AND printable ASCII (\x20-\x7E)
...
function createOdiaPDF(data, stream) {
  try {
    const { metadata = {}, lines = [], pdfLayout = 'dual', pdfTitle = '', sourceLanguage = 'English / Hindi' } = data;
    ...
    doc.font('Helvetica').fontSize(10).fillColor('#ccfbf1')
       .text('Complete YouTube Dialogue Transcript in Natural Odia', 40, 48);
    ...
    doc.text(`Original Language: ${sanitizeAsciiOnly(sourceLanguage)}  |  Non-Summarized Full Conversation`, 52, metaTop + 44);
    ...
    for (let i = range.start; i < range.start + range.count; i++) {
      ...
      doc.font('Helvetica').fontSize(8).fillColor(mutedTextColor)
         .text(`OdiaTube Transcriber  •  Full Dialogue  •  Page ${i + 1} of ${range.count}`, 40, doc.page.height - 25, {
